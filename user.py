...
class User:
    ...
    def build_a_binary_tree(self):
        ...
        BinaryTree.layer_table[1] = list(tree)
    ...
    def save(self, tree):
        try:
            ...
            if root_id == False:
                ...
                cur.execute('''insert into user_tree(tree_id_fk,user_id_fk) 
    ...
    def save_to_db(self, tree, cur):
        if tree.left_tree != None:
            left_tree_id = self.save_to_db(tree.left_tree)
        if tree.right_tree != None:
            right_tree_id = self.save_to_db(tree.right_tree)
        try:
            if hasattr(tree,'regex'):
                cur.execute('''insert into tree(node,left_tree,right_tree,regex) 
                            values(%s,%s,%s,%s)'''%(tree.node,left_tree_id,right_tree_id,tree.regex))
            else:
                cur.execute('''insert into tree(node,left_tree,right_tree) 
                            values(%s,%s,%s)'''%(tree.node,left_tree_id,right_tree_id))
    ...
    def verification(username, password):
        try:
            ...
            if result == None:
                ...
                return False
            print('登陆成功')
            return User(username, result[3], result[1])
        except MySQLdb.Error as e:
            print("发生了一些错误：",e)
            print("请重新登录")
            return False
